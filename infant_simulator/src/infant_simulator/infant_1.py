...
#state 3: stay attuned toy
    ...
    def infant_step(self, robot_pos, active_actions, wld_centers, wld_obj):
        ...
        if self.state == 2:
            ...
            if self.robot_counter == self.robot_playtime/time_step:
            ...
            if (self.infant_pos[0] < robot_pos[0]) and (self.infant_pos[1] < robot_pos[1]):
                    ...
                    random_theta = np.random.uniform(0, np.pi/2)
                    plus_10_degrees = 10 * np.pi/180
                    if theta_old >=2*np.pi:
                        theta_old = theta_old - 2*np.pi
                    if theta_old >= 0 and theta_old <= np.pi/2:
                        theta_new = theta_old
                    else:
                        if random_theta == theta_old:
                            theta_new = theta_old
                        else:
                            if random_theta - theta_old >= 0:
                                theta_new = theta_old + plus_10_degrees
                            else:
                                theta_new = theta_old - plus_10_degrees
                    ...
                    if theta_old >=2*np.pi:
                        theta_old = theta_old - 2*np.pi
                    if theta_old >= np.pi/2 and theta_old <= np.pi:
                        theta_new = theta_old
                    else:
                        if random_theta == theta_old:
                            theta_new = theta_old
                    ...
                    random_theta = np.random.uniform((3*np.pi)/2, 2*np.pi)
                    plus_10_degrees = 10 * np.pi/180
                    if theta_old >=2*np.pi:
                        theta_old = theta_old - 2*np.pi
                    if theta_old >= 3*np.pi/2 and theta_old <= 2*np.pi:
                        theta_new = theta_old
                    else:
                        if random_theta == theta_old:
                            theta_new = theta_old
                    ...
                    if theta_old >=2*np.pi:
                        theta_old = theta_old - 2*np.pi
                    if theta_old >= np.pi and theta_old <= 3* np.pi/2:
                        theta_new = theta_old
                    else:
                        if random_theta == theta_old:
                            theta_new = theta_old
            ...
            if self.toy_counter == self.toy_playtime/time_step:
                self.state_machine(wld_centers, robot_pos, wld_obj)
            ...
            if self.still_counter == 2/time_step:
                self.state_machine(wld_centers, robot_pos, wld_obj)
            print("staying still")
        elif self.state == 0:
            self.move_counter = self.move_counter + 1
            if self.move_counter == 2/time_step:
                self.state_machine(wld_centers, robot_pos, wld_obj)
            print("moving random!")
