...
class Fallback(ControlFlowNode):
    ...
    def tick_mem(self, mem, from_num = 0):

        # This is bbt algorithm 1 # for a sequence  

        if from_num >= len(self.children):
            return mem
...
#     def tick_mem(self, traversal_count, mem, from_num = 0):
# ??? not done, this method is the old version of tick_mem above
#         # This is bbt algorithm 1 # for a fallback
...
#         mem = self.children[from_num].tick(True, traversal_count)

#         # mem_given_success: subset of belief state consistent with SUCCESS
#         # mem_given_failure: subset of belief state consistent with FAILURE

#         mem_given_success, mem_given_no_success = mem.split_by_return_status(r = ReturnStatus.FAILURE)
#         self.children[from_num].tick(False, traversal_count)


#         if from_num + 1 < len(self.children):

#             mem = self.tick(active, traversal_count, from_num = from_num + 1)
...
class Sequence(ControlFlowNode):
    ...
    '''
    # Equivalent tick test, to match recursion style in BBT paper
    def tick(self, active, traversal_count, from_num = 0):
        ...
        if not active:
            ...
            if self.status == ReturnStatus.SUCCESS:
                self.children[from_num].tick(True, traversal_count)
                self.status = self.children[from_num].status
            else:
                self.children[from_num].tick(False, traversal_count)
        ...
        if from_num + 1 < len(self.children):
            ...
            self.tick(active, traversal_count, from_num = from_num + 1)
    '''
...
class Skipper(ControlFlowNode):
    label = '=>' 

    # Returns success/failure if any child does
    # Otherwise, if child is running, continues to next child
    ...
    def tick(self, active, traversal_count):
        if not active:
            ...
            for child in self.children:
                if self.status == ReturnStatus.RUNNING:
                    child.tick(True, traversal_count)
                    self.status = child.status
                else:
                    child.tick(False, traversal_count)
    ...
    def tick_mem(self, mem, from_num = 0):
        ...
        # This is bbt algorithm 1 # for a skipper  
    ...
    #     mem = self.children[from_num].tick_mem(mem, from_num)

    #     mem_given_success, mem_given_no_success = mem.split_by_return_status(r = ReturnStatus.RUNNING)
    #     self.children[from_num].tick_mem(mem, from_num)


    #     if from_num + 1 < len(self.children):

    #         mem = self.tick(mem, from_num = from_num + 1)
